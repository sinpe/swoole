...
class DefaultEventsProvider
{
    ...
    public function register(Server $server, EventManager $eventManager)
    {
        ...
        // $register->set(SwooleEvent::TASK,function (\swoole_server $server, $taskId, $fromWorkerId,$taskObj)
        // {
        ...
        // $register->set(SwooleEvent::FINISH,function (\swoole_server $server, $taskId, $taskObj)
        // {
        ...
        // $register->set(SwooleEvent::PIPE_MESSAGE,function (\swoole_server $server,$fromWorkerId,$data){
        ...
        $eventManager->add(SwooleEvent::WORKER_START, function (\swoole_server $server, $workerId) {
            //为第一个进程添加定时器
            if ($workerId == 0) {
                # 启动定时器
